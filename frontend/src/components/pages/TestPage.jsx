...
export function TestPage() {
  ...
  const [questions, setQuestions] = useState(<LoadAmountOfQuestions topicId={idTopic} testId={idTest} />);
  ...
  const [currentQuestion, setCurrentQuestion] = useState(1 / Number(questions));
  const navigate = useNavigate();
  const [advice, setAdvice] = useState(null);

  const handleNext = () => {
    console.log("jkcerjerkjnc" + questions);
    if (currentIndex < test.questions.length - 1) {
      setCurrentIndex(prevIndex => {
        ...
        setTransformation('scaleX(' + (currentIndex+1)/ + Number(questions) + ')');
        setCurrentQuestion((currentIndex + 2) + Number(questions));
        return prevIndex + 1;
      });
    } else {
      setSelectedAnswer(null);
